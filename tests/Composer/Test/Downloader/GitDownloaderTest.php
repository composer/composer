...
class GitDownloaderTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testDownload()
    {
        ...
        $expectedGitCommand = $this->getCmd("git clone --depth 1 'https://example.com/composer/composer' 'composerPath' && cd 'composerPath' && git remote add composer 'https://example.com/composer/composer' && git fetch --depth=1 composer");
    ...
    public function testDownloadUsesVariousProtocolsAndSetsPushUrlForGithub()
    {
        ...
        $expectedGitCommand = $this->getCmd("git clone --depth 1 'git://github.com/composer/composer' 'composerPath' && cd 'composerPath' && git remote add composer 'git://github.com/composer/composer' && git fetch --depth=1 composer");
        $processExecutor->expects($this->at(0))
            ->method('execute')
            ->with($this->equalTo($expectedGitCommand))
            ->will($this->returnValue(1));

        $expectedGitCommand = $this->getCmd("git clone --depth 1 'https://github.com/composer/composer' 'composerPath' && cd 'composerPath' && git remote add composer 'https://github.com/composer/composer' && git fetch --depth=1 composer");
        $processExecutor->expects($this->at(2))
            ->method('execute')
            ->with($this->equalTo($expectedGitCommand))
            ->will($this->returnValue(1));

        $expectedGitCommand = $this->getCmd("git clone --depth 1 'http://github.com/composer/composer' 'composerPath' && cd 'composerPath' && git remote add composer 'http://github.com/composer/composer' && git fetch --depth=1 composer");
