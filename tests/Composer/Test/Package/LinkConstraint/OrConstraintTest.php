...
class OrConstraintTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $versionRequireStart = new VersionConstraint('==', '1.0.0');
        $versionRequireEnd = new VersionConstraint('==', '2.0.0');
        $versionProvide = new VersionConstraint('==', '1.0.0');

        $multiRequire = new OrConstraint(array($versionRequireStart, $versionRequireEnd));
    ...
    public function testMultiVersionProvidedMatchSucceeds()
    {
        $versionRequireStart = new VersionConstraint('==', '1.0.0');
        $versionRequireEnd = new VersionConstraint('==', '2.0.0');
        $versionProvideStart = new VersionConstraint('>=', '1.0.0');
        $versionProvideEnd = new VersionConstraint('<', '2.0');

        $multiRequire = new OrConstraint(array($versionRequireStart, $versionRequireEnd));
        $multiProvide = new OrConstraint(array($versionProvideStart, $versionProvideEnd));
