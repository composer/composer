...
class CurlDownloader
{
            ...
            try {
                ...
                if (CURLE_OK !== $errno || $error || $result !== CURLE_OK) {
                    ...
                    if (
                        ...
                        && (
                            in_array($errno, [7 /* CURLE_COULDNT_CONNECT */, 16 /* CURLE_HTTP2 */, 92 /* CURLE_HTTP2_STREAM */], true)
                        ...
                        $this->restartJob($job, $job['url'], ['retries' => $job['attributes']['retries'] + 1]);
                        continue;
                    }
    ...
    private function failResponse(array $job, Response $response, string $errorMessage): TransportException
    {
        ...
        $details = '';
        if (in_array(strtolower($response->getHeader('content-type')), ['application/json', 'application/json; charset=utf-8'], true)) {
