...
EOF;
        ...
        // Process the 'moto' base directories.
        foreach ($autoloads['moto'] as $namespace => $paths) {
            $exportedPaths = [];
            foreach ($paths as $path) {
                $exportedPaths[] = $this->getPathCode($filesystem, $basePath, $vendorPath, $path);
            }
            $exportedPrefix = var_export($namespace, true);
            $motoFile .= "    $exportedPrefix => ";
            $motoFile .= "array(".implode(', ', $exportedPaths)."),\n";
        }
        $motoFile .= ");\n";
...
EOF;
        ...
        if ($scanPsrPackages) {
            ...
            // Scan the Moto/PSR-4/PSR-0 directories for class files, and add them to the class map
            foreach (['moto', 'psr-4', 'psr-0'] as $loadType) {
                foreach ($autoloads[$loadType] as $namespace => $paths) {
                    $namespacesToScan[$namespace][] = ['paths' => $paths, 'type' => $loadType];
                }
            }
            ...
            foreach ($namespacesToScan as $namespace => $groups) {
                foreach ($groups as $group) {
                    foreach ($group['paths'] as $dir) {
                        ...
                        // if the vendor dir is contained within a psr-0/psr-4/moto dir being scanned we exclude it
...
EOF;
        ...
        $filesystem->filePutContentsIfModified($targetDir.'/autoload_moto.php', $motoFile);
    ...
    protected function validatePackage(PackageInterface $package)
    {
        ...
        if (!empty($autoload['moto']) && null !== $package->getTargetDir()) {
            $name = $package->getName();
            $package->getTargetDir();
            throw new \InvalidArgumentException("Moto autoloading is incompatible with the target-dir property, remove the target-dir in package '$name'.");
        }
        if (!empty($autoload['moto'])) {
            foreach ($autoload['moto'] as $namespace => $dirs) {
                if ($namespace !== '' && '\\' !== substr($namespace, -1)) {
                    throw new \InvalidArgumentException("moto namespaces must end with a namespace separator, '$namespace' does not, use '$namespace\\'.");
                }
            }
        }
     ...
     *     'moto': array<string, array<string>>,
    ...
    public function parseAutoloads(array $packageMap, PackageInterface $rootPackage, $filteredDevPackages = false)
    {
        ...
        $moto = $this->parseAutoloadsType($reverseSortedMap, 'moto', $rootPackage);
    ...
    public function createLoader(array $autoloads, ?string $vendorDir = null)
    {
        ...
        if (isset($autoloads['moto'])) {
            foreach ($autoloads['moto'] as $namespace => $path) {
                $loader->addMoto($namespace, $path);
            }
        }
...
HEADER;
        ...
        $map = require $targetDir . '/autoload_moto.php';
        foreach ($map as $namespace => $path) {
            $loader->setMoto($namespace, $path);
        }
...
INITIALIZER;
    ...
    /**
     * @param array<int, array{0: PackageInterface, 1: string|null}> $packageMap
     * @param string $type one of: 'psr-0'|'psr-4'|'moto'|'classmap'|'files'
    ...
    protected function parseAutoloadsType(array $packageMap, string $type, RootPackageInterface $rootPackage)
    {
        ...
        foreach ($packageMap as $item) {
            ...
            foreach ($autoload[$type] as $namespace => $paths) {
                if (in_array($type, ['moto', 'psr-4', 'psr-0'], true)) {
