...
EOF;
        ...
        file_put_contents($targetDir.'/php_version_check.php', $this->getPhpVersionCheck($packageMap));
...
EOF;
    ...
    protected function getPhpVersionCheck($packageMap)
    {
        $phpConstraints = array();
        foreach ($packageMap as $item) {
            list($package, $installPath) = $item;
            foreach ($package->getRequires() as $link) {
                if ('php' === $link->getTarget() && ($constraint = $link->getConstraint())) {
                    $phpConstraints[] = $constraint;
                }
            }
        }

        if (0 === count($phpConstraints)) {
            $dump = 'return true;';
        } else {
            $comparisonDumper = new ComparisonDumper();
            $dump = $comparisonDumper->dump(new MultiConstraint($phpConstraints), 'PHP_VERSION');
        }

        return <<<PHP_VERSION_CHECK
<?php

// php_version_check.php @generated by Composer
...
PHP_VERSION_CHECK;
    ...
    protected function getAutoloadFile($vendorPathToTargetDirCode, $suffix)
    {
        ...
        if ("'" === $lastChar || '"' === $lastChar) {
            ...
            $phpVersionCheckPath = substr($vendorPathToTargetDirCode, 0, -1).'/php_version_check.php'.$lastChar;
        } else {
            $autoloadPath = $vendorPathToTargetDirCode . " . '/autoload_real.php'";
            $phpVersionCheckPath = $vendorPathToTargetDirCode . " . '/php_version_check.php'";
        }
...
\$phpVersionCheck = require_once $phpVersionCheckPath;
if (!\$phpVersionCheck) {
