...
use Composer\Semver\ComparisonDumpConfig;
use Composer\Semver\ComparisonDumper;
...
EOF;
        ...
        file_put_contents($targetDir.'/platform_check.php', $this->getPlatformCheck($packageMap));
    ...
    protected function getPlatformCheck($packageMap)
    {
        ...
        foreach ($packageMap as $item) {
            ...
            foreach ($package->getRequires() as $link) {
                if (preg_match(PlatformRepository::PLATFORM_PACKAGE_REGEX, $link->getTarget()) && ($constraint = $link->getConstraint())) {
                    $versionKey = '%' . $link->getTarget() . '_version%';
                    $replacements[$versionKey] = ''; // TODO: how do we find out how to find the way to determine the package version? For 'php' it is 'PHP_VERSION', for most it is ReflectionExtension($name)->getVersion() but there are more, ZTS, 64-bit etc. See PlatformRepository
                    $dumConfigs[] = new ComparisonDumpConfig($link->getTarget(), $constraint, $versionKey);
                }
            }
        }

        $comparisonDumper = new ComparisonDumper();
        $dump = $comparisonDumper->dump($dumConfigs);

        foreach ($replacements as $versionKey => $replacement) {
            $dump = str_replace($versionKey, $replacement, $dump);
        }
...
PLATFORM_CHECK;
    ...
    protected function getAutoloadFile($vendorPathToTargetDirCode, $suffix)
    {
        ...
        if ("'" === $lastChar || '"' === $lastChar) {
            $autoloadPath = substr($vendorPathToTargetDirCode, 0, -1).'/autoload_real.php'.$lastChar;
            $platformCheckPath = substr($vendorPathToTargetDirCode, 0, -1).'/platform_check.php'.$lastChar;
        } else {
            $autoloadPath = $vendorPathToTargetDirCode . " . '/autoload_real.php'";
            $platformCheckPath = $vendorPathToTargetDirCode . " . '/platform_check.php'";
        }
...
\$phpVersionCheck = require $platformCheckPath;
if (!\$phpVersionCheck) {
