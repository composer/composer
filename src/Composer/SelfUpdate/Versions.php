...
class Versions
{
    const CHANNELS = 'stable,preview,snapshot,1,2';
    ...
    {
        if (!in_array($channel, self::getAllowedChannels(), true)) {
            throw new \InvalidArgumentException('Invalid channel '.$channel.', must be one of: ' . implode(', ', self::getAllowedChannels()));
        }
