...
class ValidatingArrayLoader implements LoaderInterface
{
    ...
    public function load(array $config, $class = 'Composer\Package\CompletePackage')
    {
        ...
        foreach (array('source', 'dist') as $srcType) {
            if ($this->validateArray($srcType) && !empty($this->config[$srcType])) {
                ...
                if ($srcType === 'source' && isset($this->config[$srcType]['type']) && $this->config[$srcType]['type'] === 'perforce'
     ...
     */
    public static function validatePackage(PackageInterface $package)
    {
        ...
        foreach ($sourceDist as $field => $value) {
            if (null !== $value && Preg::isMatch('{^\s*-}', $value)) {
        ...
        if ($package->getSourceType() === 'perforce' && null !== $sourceUrl && !Perforce::isValidPort($sourceUrl)) {
